...
export default function Checkout() {
  ...
  const answerAtDeliveryLocation = (value) => {
    ...
    setLocationError('');
    if (!value) { setCoords(null); return; }
    if (!navigator.geolocation) {
      setLocationError("Your browser doesn't support sharing location — no problem, you can still check out.");
      return;
    }
    setLocating(true);
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocationError("Couldn't get your location — no problem, you can still check out.");
        setLocating(false);
      },
    ...
    return (
      <div className="text-center py-24">
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          <button onClick={() => navigate('/login')} className="px-8 py-3 rounded-full bg-ink text-white text-sm hover:bg-green transition-colors">
            Sign in
          </button>
          <button onClick={() => setCheckingOutAsGuest(true)} className="text-sm text-black/60 hover:text-green underline underline-offset-2">
            Continue as guest
          </button>
        </div>
      </div>
  ...
  const placeOrder = async (e) => {
    ...
    try {
      let data;
      const locationFields = coords ? { latitude: coords.lat, longitude: coords.lng } : {};
      if (user) {
        ...
        ({ data } = await api.post('/orders', { ...delivery, shippingCost, ...locationFields }));
      } else {
      ...
      if (err.response?.status === 401) {
        ...
        toast('Your session expired — please sign in again or continue as guest.', 'error');
      } else {
  ...
  return (
    <div className="w-full mx-auto max-w-lg px-4 py-12">
      ...
      <form onSubmit={placeOrder} className="mt-8 space-y-4">
        ...
        <div className="bg-white border border-black/5 rounded-lg p-4 space-y-3">
          <p className="text-xs uppercase tracking-[0.2em] text-green">Delivery Details</p>
          <label className={labelClass}>RGMC Link/Block, Flat/House No. *
            <input
              required
              value={delivery.area}
          ...
          <div className="pt-1">
            <p className={labelClass}>Are you checking out from where this should be delivered?</p>
            <div className="mt-1.5 flex gap-2">
              <button
                type="button"
                onClick={() => answerAtDeliveryLocation(true)}
                className={`px-4 py-2 rounded-full text-sm border transition-colors ${
                  atDeliveryLocation === true ? 'bg-green text-white border-green' : 'border-black/15 hover:border-green hover:text-green'
                }`}
              >
                Yes
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm border transition-colors ${
                  atDeliveryLocation === false ? 'bg-ink text-white border-ink' : 'border-black/15 hover:border-black'
                }`}
              >
                No
              </button>
            </div>
            {locating && <p className="mt-1.5 text-xs text-black/40">Getting your location…</p>}
            {coords && (
              <p className="mt-1.5 flex items-center gap-1 text-xs text-green">
                <MapPin size={12} strokeWidth={2} /> Location shared — this helps your rider find you faster.
              </p>
            )}
            {locationError && <p className="mt-1.5 text-xs text-black/40">{locationError}</p>}
          </div>
        </div>
